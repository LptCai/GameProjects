//教学关卡
class Gmap extends egret.Sprite{
    public constructor() {
        super();
        this.init();
    }
    private role: Role;//角色
    private walls: Obstacle[];//墙壁
    private balls: MemoryBall[];//记忆球
    private door: TransGate;//传送门
    private textArea: egret.TextField;//界面顶部文字信息
    private bg: egret.Bitmap;//关卡背景图
    private Gcontrol: ControlArea;//操作控制区
    private music: Music;//音乐按钮
    private getui: egret.Bitmap;//ui界面按钮
    public _timer0: egret.Timer;//控制全局画面刷新
    public _timer1: egret.Timer;//控制墙壁刷新
    private ateballnum: number=0;//记录记忆球是否完全被吃掉
   ...
   //ui按钮的控制
    ...
    { 
       ...
       if(this.door != null) {
        ...
        if(this.ateballnum == 3) {
            ...
            this.ateballnum = 0;
            //上方生成新墙壁：
            var wall8: Obstacle = new Obstacle(0,-120,100,true,3,-120,600);//可移动的墙壁
    ...
    {        
       //绘制障碍物
        ...
        this.walls = [];
        var wall0: Obstacle = new Obstacle(0,120,200,false,0,0,0);//横上左
        var wall1: Obstacle = new Obstacle(0,240,200,false,0,0,0);//横上右
        var wall2: Obstacle = new Obstacle(0,120,470,false,0,0,0);//横下左
        var wall3: Obstacle = new Obstacle(0,240,470,false,0,0,0);//横下右
        var wall4: Obstacle = new Obstacle(1,90,230,false,0,0,0);//竖左上
        var wall5: Obstacle = new Obstacle(1,90,350,false,0,0,0);//竖左下
        var wall6: Obstacle = new Obstacle(1,360,230,false,0,0,0);//竖右上
        var wall7: Obstacle = new Obstacle(1,360,350,false,0,0,0);//竖右下
        ...
        //绘制记忆球
        this.balls = [];
        var ball0: MemoryBall = new MemoryBall(320,280,0,0);
        var ball1: MemoryBall = new MemoryBall(180,230,0,0);
        var ball2: MemoryBall = new MemoryBall(120,300,0,0);
